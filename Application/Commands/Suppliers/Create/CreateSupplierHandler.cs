...
namespace Application.Commands.Suppliers.Create
{
    public class CreateSupplierHandler(IUnitOfWork unitOfWork,
        ...
        IMediator mediator) : IRequestHandler<CreateSupplierCommand, Result<TwoFactorSetupDto>>
    {
        public async Task<Result<TwoFactorSetupDto>> Handle(CreateSupplierCommand request, CancellationToken cancellationToken)
        {
            ...
            var user = new User(Email.Create(request.Model.Email),password,base64Salt,PhoneNumber.Create(request.Model.PhoneNumber),
                request.Model.Gender,DateOfBirth.Create(request.Model.Dob),fullName);

            var supplier = new Supplier(user.Id,request.Model.CompanyName,PhoneNumber.Create(request.Model.PhoneNumber),
               Email.Create(request.Model.Email) ,request.Model.PhoneNumber, request.Model.Address, request.Model.TaxId);

            user.AddRole(Role.Supplier);
            ...
            try
            {
                ...
                "Customer",
                supplier.Id,
                $"Supplier '{user.FullName}' was created.",
                request.RequestMetadata.IpAddress,
                request.RequestMetadata.UserAgent
                ));
            ...
            catch (Exception ex)
            {
                var failureLog = new AuditLog(
                   ...
                   entityName: "Supplier",
                   entityId: null,
                   details: $"Supplier creation failed for email '{request.Model.Email}': {ex.Message}",
                ...
                return Result<TwoFactorSetupDto>.Failure("An unexpected error occurred while creating the Supplier."); ;
            }
        }
    }
}
