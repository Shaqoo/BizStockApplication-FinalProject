...
namespace Application.Commands.DeliveryAgents.Create
{
    ...
    public class CreateDeliveryAgentHandler(IUserRepository userRepository,
        IAuditLogRepository logRepository,
        IUnitOfWork unitOfWork,
        IDeliveryAgentRepository deliveryAgentRepository,
        ILogger<CreateDeliveryAgentHandler> logger,
        IMfaService mfaService,
        IRecoveryCodeGenerator recoveryCodeGenerator,
        IMediator mediator,
        IMemoryCacheService memoryCacheService,
        IPublishEndpoint pubishEndpoint) : IRequestHandler<CreateDeliveryAgentCommand, Result<TwoFactorSetupDto>>
    {
        public async Task<Result<TwoFactorSetupDto>> Handle(CreateDeliveryAgentCommand request, CancellationToken cancellationToken)
        {
            ...
            var fullName = $"{request.Model.FirstName.Trim()} {request.Model.LastName.Trim()}".Trim();
            ...
            user.AddRole(Role.DeliveryAgent);
            ...
            var deliveryAgent = new DeliveryAgent(fullName,Email.Create(request.Model.Email),request.Model.VehicleNumber,request.Model.PhoneNumber);

            try
            {
                await unitOfWork.BeginTransactionAsync();
                await userRepository.AddAsync(user);
                await deliveryAgentRepository.AddAsync(deliveryAgent);
                ...
                "Customer",
                deliveryAgent.Id,
                $"DeliveryAgent '{user.FullName}' was created.",
                request.RequestMetadata.IpAddress,
                request.RequestMetadata.UserAgent
            );
                ...
                await mediator.Publish(new UserRegisteredEvent(user.Id, (string)user.Email, fullName, mfa.ManualEntryKey, mfa.QrCodeImageUrl));
            ...
            catch (Exception ex)
            {
                var failureLog = new AuditLog(
                   userId: Guid.Empty,
                   action: "CreateDeliveryAgentFailed",
                   entityName: "DeliveryAgent",
                   entityId: null,
                   details: $"DeliveryAgent creation failed for email '{request.Model.Email}': {ex.Message}",
                ...
                return Result<TwoFactorSetupDto>.Failure("An unexpected error occurred while creating the DeliveryAgent.");
            }
        }
    }
}
